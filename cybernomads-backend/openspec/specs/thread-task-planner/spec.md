...
Define the MVP thread task planner that discovers executable tasks for running traffic works, evaluates lightweight task conditions, and dispatches accepted tasks through the Agent service abstraction while preserving task, traffic work, and Agent provider boundaries.
