# traffic-work-contracts Specification

## Purpose

Define the top-level API and SQL contracts for the traffic work domain so later runtime implementation has a stable source of truth for work identity, binding, lifecycle state, and context readiness.
...
#### Scenario: Update traffic work without changing aggregate identity

- **WHEN** the contracts define traffic work update behavior
- **THEN** the contracts SHALL preserve the original traffic work identity
- **AND** the contracts SHALL allow changing display name, product reference, strategy reference, and object bindings without creating a second traffic work object
...
- **AND** the list response MAY include product and strategy summary information
...
#### Scenario: Update paused or ready traffic work
...
### Requirement: Traffic work contracts SHALL keep object binding aggregated in MVP SQL

The system SHALL keep traffic work SQL contracts focused on work-level references and aggregated object binding semantics instead of prematurely introducing an object-binding relation model.

#### Scenario: SQL contract stores aggregated binding set

- **WHEN** the SQL contract is created
- **THEN** it SHALL store one aggregated object binding set for each traffic work
- **AND** it SHALL avoid requiring a dedicated traffic-work object-binding relation table in MVP

#### Scenario: SQL contract preserves product and strategy boundaries

- **WHEN** the SQL contract is created
- **THEN** it SHALL store stable product and strategy references rather than copied product or strategy content
- **AND** it SHALL avoid persistence semantics that pull product-domain or strategy-domain full-content concerns into the traffic work table
...
#### Scenario: Contract scope stays aligned with architecture boundary

- **WHEN** the contracts are reviewed against current architecture and domain documentation
- **THEN** they SHALL remain within the traffic-work domain boundary described for work management
- **AND** they SHALL avoid mixing scheduler implementation concerns into the top-level traffic work contracts
