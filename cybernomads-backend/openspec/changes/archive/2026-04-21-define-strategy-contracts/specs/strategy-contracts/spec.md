...
The system SHALL ensure strategy top-level contracts reflect the current strategy domain design, where a strategy is an independent markdown asset with minimal structured metadata, inline snapshot markers, and typed object placeholder declarations.
...
- **AND** it SHALL NOT introduce deletion, draft, publish, archive, version-chain, experiment, or runtime performance semantics

#### Scenario: API contract fixes typed placeholder syntax
- **WHEN** the API contract is created
- **THEN** it SHALL describe object placeholders using the `{{objectType:objectKey}}` syntax as the stable strategy-body convention
- **AND** it SHALL avoid using display-name-based placeholder syntax as the normative contract language
