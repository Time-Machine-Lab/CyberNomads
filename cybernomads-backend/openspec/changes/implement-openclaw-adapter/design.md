...
同时，这个提案面对的是外部服务适配问题，而不是完全自控的内部模块开发，因此实现前必须先做一次接口调研和本地摸底，确认 OpenClaw 当前真实可用的 API、鉴权方式与能力边界，再进入代码实现。
...
**Goals:**
- 先完成 OpenClaw 接口调研、能力摸底与本地最小闭环验证。
...
**Decision**
- 在进入 OpenClaw 适配实现前，先完成接口调研、本地连通验证和能力映射分析，并把发现结果作为适配实现的前置输入。

**Rationale**
- OpenClaw 属于外部服务，接口边界、鉴权方式和部分能力形态如果仅靠猜测实现，后续返工概率很高。
- 先做 research spike，可以更早发现 provider 端口定义与 OpenClaw 实际能力之间的缺口。

**Alternatives Considered**
- 直接根据当前设想开始编码。
  - 放弃原因: 很容易在会话、history、subagent 或 capability provisioning 上出现语义错配。

### 3. 通过单独 client/gateway 封装 OpenClaw 交互细节
...
### 5. 错误语义统一映射到 provider-neutral 结果
...
- [OpenClaw 实际接口可能存在调研偏差] -> 先执行 research spike，再进入实现；如接口与设想不符，优先修正设计与端口映射。
- [prompt 驱动的能力准备结果可能不稳定] -> 上层只依赖显式成功/失败状态，不依赖 provider 返回的自由文本语义。
- [首期只支持 OpenClaw 会让抽象看起来“只有一个实现”] -> 通过端口和错误映射持续保持抽象边界，避免为单实现放弃解耦。
- [调研阶段会拉长首轮交付节奏] -> 接受这一前置成本，以换取实现阶段更少返工和更清晰的 provider 边界。

## Migration Plan

1. 调研 OpenClaw 当前接口文档、源码或可用说明，确认鉴权、会话、历史、subagent 与能力准备入口。
2. 使用本地 OpenClaw 环境完成最小连通验证，记录关键请求/响应样例。
3. 对齐 Agent 服务接入模块定义的 provider 端口能力，输出 OpenClaw 能力映射和差异清单。
4. 在 `src/adapters/agent/openclaw/` 中实现 OpenClaw client/gateway 封装。
5. 实现连通性检查、会话相关能力、subagent 调用和能力准备能力。
6. 将 OpenClaw 适配器注册到 Agent 服务接入模块的 provider 装配流程中。
7. 通过本地 OpenClaw 环境完成最小集成验证。

## Open Questions

- OpenClaw 当前稳定可用的会话、历史查询和 subagent 接口边界，需要在真正实现前做一次接口确认。
- 凭证模式是统一走 header、query 还是其他鉴权方式，需要在适配实现前锁定。
- 如果 OpenClaw 的实际能力与当前 provider 端口存在缺口，是应先调整端口抽象，还是在适配层增加兼容包装，需要在调研后再判断。
