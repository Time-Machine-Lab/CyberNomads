...
--   4. 当前阶段不把任务拆分草案、Review、确认快照、调度条件、执行日志、平台脚本细节或运行统计混入本表。
--      任务拆分过程产物由 `task_decomposition_runs` 与 `task_decomposition_artifacts` 承担。
...
--
-- context_preparation_status:
--   - `pending`: 尚未完成上下文准备、正在生成草案/Review，或 Review 通过后等待用户确认。
--   - `prepared`: 用户确认任务方案且系统提交正式任务集后，当前工作上下文已准备完成，可为启动提供前提。
--   - `failed`: 最近一次上下文准备失败。
--
-- context_prepared_at:
--   - 最近一次上下文准备成功完成的时间。
...
-- 3. 当前阶段不在本表定义产品快照、策略快照、任务方案草案、Review 报告或用户反馈；
--    这些产物由对应领域或 `task_decomposition_runs` / `task_decomposition_artifacts` 承担。
