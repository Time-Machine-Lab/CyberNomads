...
-- 范围：
--   1. 策略以稳定主键 `strategy_id` 作为唯一标识，当前新建策略默认采用 8 位短标识。
...
--   5. 导入块标记属于 Markdown 正文的一部分，不单独建关系表；当前推荐短契约 `<!-- s:strategyId --> ... <!-- /s -->`。
...
--   - 策略稳定标识。
--   - 由系统生成，不依赖策略名称。
--   - 当前新建策略默认采用 8 位十六进制短标识；历史长 UUID 继续兼容。
--
-- name:
--   - 策略可读名称。
...
-- 4.1 导入块标记当前推荐短契约 `<!-- s:strategyId --> ... <!-- /s -->`，
--     历史长契约继续兼容读取。
