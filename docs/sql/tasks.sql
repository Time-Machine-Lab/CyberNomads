...
--   2. 任务由 Cybernomads Agent 生成草案、Review 通过并经用户确认后，
--      通过任务模块受控入口创建或替换；普通用户不直接逐条手工创建完整任务集合。
...
--    工具或服务入口完成。
-- 7. OpenClaw 只执行已确认单任务；它产生的执行异常可作为未来拆分反馈写入
--    `task_decomposition_artifacts`，但不允许 OpenClaw 自行重拆整份引流工作。
