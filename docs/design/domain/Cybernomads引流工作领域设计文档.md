...
- **任务拆分运行批次 (Task Decomposition Run)**：Cybernomads Agent 围绕某个引流工作生成草案、Review、修正、报告、等待确认和系统提交的可追踪过程。
- **用户确认 (User Confirmation)**：用户认可 Review 通过后的任务方案，并授权系统将确认快照提交为正式任务集的动作。
- **反馈重拆 (Feedback-based Replanning)**：用户基于拆分报告、Review 问题或执行异常提交反馈，触发新的任务拆分运行批次。
...
- **准备先于运行约束**：引流工作必须先完成一次任务准备，才能具备可启动前置条件。任务准备只有在 Review 通过、用户确认且系统正式任务落库成功后才可标记为 `prepared`。
...
- **确认前隔离约束**：在任务方案 Review 通过并等待用户确认之前，不得写入正式任务表，也不得把 `context_preparation_status` 标记为 `prepared`。
...
## 6. Cybernomads Agent 拆分批次补充（2026-05-20）

创建或更新引流工作后，系统不再把整份引流工作交给 OpenClaw 自由拆分和落库，而是启动一个任务拆分运行批次：

```text
TrafficWork context -> Cybernomads Agent draft -> Review/Repair -> report
                    -> waiting_user_confirmation -> system commit -> prepared
```
