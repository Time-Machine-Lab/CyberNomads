...
- **确认快照 (Confirmation Snapshot)**：用户确认的任务方案结构化快照，是系统写入正式任务集的唯一来源。
...
- **创建来源约束**：任务由 Cybernomads Agent 基于引流工作内容生成草案，经过 Agent Review 与用户确认后，由系统编排层通过任务模块创建；普通用户不直接手工编写完整任务集合。
...
## 5. 确认后落库补充（2026-05-20）

本期任务域继续保持正式任务集的唯一写入边界，但写入来源从“外部 Agent 调工具自由落库”调整为“用户确认后的系统提交”：

- Cybernomads Agent Planner 只生成结构化任务方案草案。
- Review Agent 负责判断草案是否通过、需修正或失败。
- 用户确认后的确认快照是任务模块创建或替换正式任务集的来源。
