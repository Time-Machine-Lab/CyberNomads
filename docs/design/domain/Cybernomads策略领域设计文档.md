...
- **包含**：识别策略正文中的对象引用占位符与导入块标记，并向上层提供可编译的策略输入。
- **包含**：向引流工作域、任务规划服务或后续对象绑定流程提供完整策略正文、去标记后的提示词模板和占位符清单。
...
在 Cybernomads 的当前阶段，策略域不是一个复杂的流程编排器，也不是一个策略关系图数据库。它更像一个稳定的“策略提示词资产中心”，负责回答三个核心问题：当前有哪些可被选择的策略资产、某条策略的完整 Markdown 正文是什么、这份正文中声明了哪些可供后续绑定的对象引用。
...
- **对象引用占位符 (Object Placeholder Token)**：在策略正文中声明后续需要绑定资源的位置标记，当前建议采用 `{{objectType:objectKey}}` 语法，例如 `{{account:backup_account}}`。
- **对象类型 (Object Type)**：占位符左侧的资源类别标识，例如 `account`、`asset`、`product`。
- **对象稳定键 (Object Key)**：占位符右侧的稳定标识符，用于表达“需要哪一个对象”，不依赖展示名。
- **可编译策略输入 (Compilable Strategy Input)**：策略域在去除编辑辅助标记后，对外提供的干净提示词模板，可继续进入对象绑定与任务规划流程。
- **占位符清单 (Placeholder Set)**：从策略正文中解析出的对象引用占位符集合，供上层绑定流程使用。
...
- `PlaceholderTokenSet` 是从正文中派生出的值对象，用于表达当前策略声明了哪些待绑定对象引用。

在领域语义上，`Strategy` 的核心职责不是维护复杂引用图，而是保证“一条策略始终对应一份完整、独立、可解析的 Markdown 正文”，并且能够从这份正文中稳定提取编辑辅助标记与对象引用声明。
...
- **编译净化约束**：当对外提供可编译策略输入时，策略域应去除导入块标记等编辑辅助注释，但保留实际正文语义内容。
- **占位符语法约束**：对象引用必须使用稳定、可解析的统一语法 `{{objectType:objectKey}}`，不得依赖展示名或页面临时文本来表达绑定目标。
- **稳定标识约束**：对象引用占位符中的 `objectKey` 必须是稳定键，而不是“账号A”“二维码图片A”这类易变展示名。
- **绑定解耦约束**：策略域只负责识别和暴露占位符，不负责将占位符绑定到具体账号、素材或产品实例。
...
- **用户故事 4**：作为用户，我希望在策略正文中声明对象引用占位符，以便后续流程能够把账号、素材或产品等资源绑定到这条策略的具体位置。
  - **验收标准 (AC)**：系统能够稳定识别 `{{objectType:objectKey}}` 形式的占位符，并在查询或编译时提供占位符清单。

- **用户故事 5**：作为上层引流工作或任务规划使用方，我希望获取一份去除了编辑辅助标记的可编译策略输入，以便后续流程基于干净提示词继续执行对象绑定和任务规划。
  - **验收标准 (AC)**：当系统请求可编译策略输入时，返回内容不包含导入块标记，但保留完整策略正文语义和占位符声明。
...
- **命令 (Command)**：`CompileStrategyInputCommand`（获取可编译策略输入）
...
- **事件 (Event)**：`StrategyCompiledEvent`（可编译策略输入已生成）
...
sequenceDiagram
    ...
    StrategyDomain->>StrategyDomain: 解析导入块标记与对象占位符
    StrategyDomain-->>Editor: 返回策略详情与最新解析结果

    WorkOrPlanner->>StrategyDomain: 请求可编译策略输入
    StrategyDomain->>StrategyDomain: 去除导入块标记并提取占位符清单
    StrategyDomain-->>WorkOrPlanner: 返回干净提示词模板 + PlaceholderSet
```

这条核心行为流表达的是策略域当前最重要的稳定闭环：
- 用户可以把其他策略整篇插入当前策略，但插入的本质是内容快照，而不是跨策略依赖。
- 编辑器中的导入块标记会随着正文一起保存，从而支持后续继续识别来源区块。
- 当上层消费策略时，策略域负责输出去除编辑辅助标记后的可编译输入，并暴露占位符清单供后续绑定流程使用。
