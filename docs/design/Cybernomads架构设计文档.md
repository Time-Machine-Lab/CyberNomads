...
- **Agent 服务提供方**：当前接入 OpenClaw，未来可切换为其他外部 Agent 服务或自研 Agent 服务。
- **平台执行能力**：由外部 Agent 服务借助 Skill 或等价能力调用平台交互脚本。架构上支持多平台，当前仅实现 B 站脚本适配。
- **结构化存储**：当前默认使用 SQLite 承载元数据和状态索引，未来可切换为 MySQL、Redis 或其他介质。
- **非结构化存储**：当前默认使用本地文件系统承载 Markdown 原文、日志原文和执行产物引用，未来可切换为对象存储或远端文件服务。
- **本地操作系统环境**：需要兼容 macOS、Windows、Linux 三类主流桌面环境，尤其是文件路径、权限模型和大小写敏感差异。
...
flowchart LR
    ...
    Agent["Agent 服务提供方<br/>当前: OpenClaw"]
    Skill["平台执行能力 / Skill"]
...
- 用户启动某个引流工作。
- 系统调度器持续判断哪些任务满足执行条件。
- 上层应用编排调用外部 Agent 服务的原子能力来执行任务，并通过系统工具回写状态和日志。
- 前端执行台持续展示任务流和日志时间线。
...
flowchart TB
    ...
    Domain["业务域层<br/>引流工作 / 任务 / 策略 / 对象绑定 / Agent服务接入"]
...
- 策略组装与任务生成服务：把策略包组合后的完整提示词交给外部 Agent 服务，生成任务集合
- 任务调度服务：负责当前 MVP 的线程决策
- 任务分发服务：把可执行任务交给 Agent 服务抽象层
- 日志与观察服务：统一汇总状态、日志和执行摘要供前端查询

需要特别强调的是：任务规划请求与任务执行请求属于应用服务层对 Agent 原子能力的组合使用，而不是 Agent 服务接入域自身的领域职责。Agent 服务接入域负责“接谁、怎么连、是否已准备好”，应用服务层负责“拿它来做什么”。

### 2.3 业务域层：核心业务边界
业务域层承载稳定的业务概念，而不在这里展开高变动的字段级细节。当前建议固定以下边界：

- **Agent 服务接入域**
  负责外部 Agent 服务接入方式、连接状态、当前激活服务与能力准备状态。
...
- **AgentPort**
  负责连接外部 Agent 服务，并向上层暴露统一调用入口。当前由 OpenClawAdapter 实现。任务规划、消息发送、会话管理、subagent 调用等原子能力由具体适配器承接，但这些能力在业务上由应用服务层组合使用。

- **SkillProvisionPort**
  负责检查、准备、更新 Cybernomads 运行所需的能力。
...
与此同时，文件存储设计必须从一开始就按跨平台约束处理，而不是默认按某一个操作系统实现。当前至少需要满足以下约束：
- 文件存储路径必须由统一的路径抽象生成，不能在业务流程中写死 Windows 盘符、反斜杠或某个平台专有目录。
- 默认存储根目录为产品启动时的当前目录下 `cybernomads/` 文件夹；后续可以保留通过配置覆盖的扩展能力，但 MVP 的默认行为必须稳定一致。
- 文件名与目录名应避免使用跨平台保留字符、保留文件名和大小写歧义写法，保证在大小写敏感与不敏感文件系统上都可稳定工作。
- 日志、Markdown 原文和执行产物的读写应避免依赖平台特定换行符和权限假设。
- 如果未来从本地文件系统切换到对象存储，当前目录层级语义仍应保持稳定，便于平滑迁移。
...
│   └── xxxx策略.md
└── work/                         # 引流工作目录
    └── xxxx工作/                 # 具体引流工作目录，同时作为 Agent 执行时的上下文目录
        ├── scripts/              # 该引流工作所需脚本目录
        └── task.md               # 当前引流工作任务描述与上下文入口文件
```

这个目录结构在架构上的语义如下：
- `product/` 承载产品 Markdown 原文，是产品域在本地文件系统中的原文映射。
- `strategy/` 承载策略 Markdown 原文，是策略域在本地文件系统中的原文映射。
- `work/` 承载引流工作的上下文目录，每个具体工作目录都对应一个独立的执行上下文。
- `work/<具体工作>/` 不是普通缓存目录，而是面向 Agent 执行的工作上下文空间，Agent 在执行时应以该目录下内容作为输入上下文之一。
- `scripts/` 与 `task.md` 共同构成当前引流工作的最小执行载体，其中 `task.md` 更偏任务入口与说明，`scripts/` 更偏执行辅助资产。
...
- 任务规划是应用服务层基于 Agent 服务原子能力形成的上层业务行为，而不是 Agent 服务接入域自身的职责。
...
sequenceDiagram
    ...
    loop 每秒轮询
        ...
        Dispatcher->>AP: 请求外部 Agent 服务提供执行原子能力
        AP->>SA: 下发任务提示与上下文
        SA->>Tools: 回写状态 / 日志 / 摘要
        Tools->>MS: 统一落库或写入文件
        MS-->>UI: 提供最新任务流与日志
...
- Agent 服务域负责提供统一外部执行入口；“何时请求任务规划”“何时请求任务执行”由应用服务层决定。
...
flowchart LR
    ...
    Agent["OpenClaw Endpoint<br/>当前 Agent 服务接入实现"]
...
除此之外，本地运行形态还增加一条工程约束：
- **文件系统兼容 macOS、Windows、Linux**：包括启动目录、数据目录、SQLite 文件路径、日志目录和产物目录都必须可跨系统工作。
- **启动即创建本地项目目录**：系统启动后需在当前目录下创建 `cybernomads/` 本地项目目录，并按既定结构组织产品、策略和引流工作上下文文件。
...
- **后果**：当前实现需要多一层抽象，短期编码量略有增加，但换来后续迁移成本显著降低；同时文件系统实现必须从第一天就按 macOS、Windows、Linux 兼容要求设计。

**决策：Agent 层通过端口解耦，subagent 不直接操作数据库**
- **背景**：当前接入 OpenClaw，未来可能切换到远端 Agent 或本地 Agent；同时任务执行中需要频繁更新状态和日志。
- **决策**：外部 Agent 服务只通过统一端口接入，subagent 只能调用系统工具更新状态和日志，而不能直接访问数据库。
- **理由**：这样 Agent 服务实现层与存储层都保持独立，后续无论更换 Agent 服务还是更换存储介质，都不会破坏核心业务模型。
...
- **决策**：任务在引流工作创建或更新时由应用服务层调用外部 Agent 服务原子能力一次性生成；运行时由调度器基于执行条件判断是否执行。
